...
def parse(soup, type_="accept-oral"):
    # 创建一个文件夹保存下载的论文
    if not os.path.exists(f"ICLR_2024_Papers/{type_}"):
        os.makedirs(f"ICLR_2024_Papers/{type_}")
    base_dir = f"ICLR_2024_Papers/{type_}"

    soup = soup.find("div", id=type_)
    ...
    # 下载
    error_list = []
    for paper_info in tqdm.tqdm(paper_list_info):
        try:
            # 格式化标题
            formatted_title = sanitize_title(paper_info["title"])
            # 构建保存路径
            save_path = os.path.join(base_dir, f"{formatted_title}.pdf")
            # 下载PDF文件
...
if __name__ == "__main__":
    ...
    file_path = "./pages/spotlight_1.html"
    file_list = os.listdir("./pages")
    for file_path in file_list:
        if file_path.endswith(".html"):
            with open(f"./pages/{file_path}", "r", encoding="utf-8") as file:
                html_content = file.read()
            print(file_path)
            # 使用BeautifulSoup解析HTML内容
            soup = BeautifulSoup(html_content, "html.parser")
            if "oral" in file_path:
                parse(soup, type_="accept-oral")
            elif "spotlight" in file_path:
                parse(soup, type_="accept-spotlight")